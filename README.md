...
These are one unit composing itself, and intra-unit imports carry no restriction beyond layer rank, which binds wherever the files sit. A different name is a different unit, and the cross-unit rules (the lateral service-to-service and adapter-to-adapter smells among them) act on the names: to draw a boundary between two things sharing a name, give each its own. A unit is a concern as much as a subdomain is; the name carries the unit, the graph draws the subdomain, and a directory is how the files are grouped. Group files however a crowded domain reads best, a grouping that stays free because the classification reads the name and the graph. A concern becomes a subdomain when it grows a surface, the thing a reference crosses to enter it. The spec never mandates decomposition; structure starts coarse and splits when a boundary becomes real. An effect-only import (a bare import for its side effect) reaching past its unit hides an effect in the graph and is a smell; effect composition by import belongs to the composition root.

### Slicing direction

A domain's files partition one way or the other. **Vertical** slicing is concern-first: units named for their concern, each file carrying its layer as a name suffix, the layers meeting inside each concern's column. **Horizontal** slicing is rank-first: directories named for their layer, each holding plain concern-named files, the concerns meeting inside each layer's row. Both are spellings of the same classification - the layer rides the suffix in one and the rank-named directory in the other, the unit rides the concern name in both - and every rule reads the classification, which both spellings carry in full.

The choice is judgment the architecture leaves to the domain, one domain at a time. Highly specialized concerns slice vertically: each concern owns its column, and a boundary between two concerns is a rename away. Logic entangled by design - pieces leaning on one another across ranks, as a library's internals routinely do - slices horizontally: the rank rows carry the structure, and the imports cross inside a row. A concern partition over the same code would put a wall through every lean.

A wrong slice shows up in the graph: imports that climb the rank order accumulate between sibling pieces, and the cluster is the pressure of a partition fighting its dataflow. The remedy is to re-slice - merge the fighting siblings into one piece whose rank rows carry the structure, or re-rank the pieces so every import points down. The machine gathers the pressure and surfaces the cluster; choosing the new slice is the reviewer's decision, and the enforcement tiers place it there.
