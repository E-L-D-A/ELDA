...
- **Predictable location.** The kind of an effect predicts its layer (a binding bug in Harnesses, a rule bug in Axioms, an I/O bug in Services, an orchestration bug in Flows), and its concern predicts its domain. Navigation is "go to a predicted location".
...
Placement is a claim the graph adjudicates. Because the structure is the graph, a file's name and position are an assertion about where it sits, and the graph is the fact that assertion answers to. Most architectural approaches read structure off the file tree and stop there: a folder is a layer, a path is a domain, and validation asks only whether a file sits where its kind belongs; the most thorough add a shallow next-door import check and a cycle pass, and none reads the graph the imports actually form. ELDA checks the claim against the resolved graph - the suffix claims a layer, the surface claims a boundary, and the enforcement follows each binding to where it truly lands and what it truly crosses. A claim the graph contradicts is the finding, so a boundary is surface ownership computed from the edges and a layer is validated against its landings; the directory tree is only where a developer chose to keep the files.
...
ELDA's parts are prior art: Clean Architecture's four layers with the outer ring re-purposed, Hexagonal's primary and driven ports as the API and Events surfaces, DDD's bounded contexts replicated per concern, Seemann's Composition Root and Pure DI, functional-core-imperative-shell extended to a two-tier impure boundary (Harnesses, then the shell), the observer/iterator duality behind Stream/Generator, the colored-function diagnosis with a decoloring response, and errors-as-values without the negative-branch bias.
...
| **Harnesses** | green | Coupling gear between the domain and every shape it does not define: translation, outcome conversion, host bindings | UI bindings, request interceptors, outcome translators, presenters |
| **Flows** | red | Business/application logic: processes, watchers, pipelines | Feature workflows, data transformations, event handlers |
| **Axioms** | yellow | Pure, framework-free domain invariants and vocabulary | UX rules, data constraints, network policy |

The four layers face two audiences. A domain's **Services** are reached by the **composition root**, which instantiates and wires them; a **peer domain** reaches only the domain's **Flows** and vocabulary, through the public surface, never its Services (one graded exception, the peer mounting treated under Composition roots).\
Hence the naming, each name stating the thing's own law:

- **Axioms** are what the domain assumes and everything above derives from: the primitive vocabulary and the invariant rules, derived from nothing - the layer's dependency-freedom spelled as a name. An axiom system is primitives plus postulates, so passive vocabulary (shapes, tokens, prose, templates) belongs here exactly as the rules do.
- **Flows** advance the domain's processes by settled values: watchers, pipelines, the Pipe-Event-Stream cycle. The name says what peers consume, and it says the layer's texture - synchronous, stateful, error-by-value steps.
- **Harnesses** are the coupling gear between the flows and the world the services touch, converting what mismatches and absorbing what throws, the way a wiring harness couples components while keeping each replaceable.
- **Services** are domains *served* up for the runtime, composed by the root.

One sentence carries the stack: axioms hold the settled rules, flows advance by settled values, harnesses settle the unsettled shapes, and services drive the unsettled world - **settled** meaning synchronous, exception-free, and typed: a value already in hand.

The four names sort alphabetically into the dependency order, so every file listing reads as the stack and a file imports only from layers listed above it, in both slicings (the diagrams draw the same order from the other end, Services on top). For readers arriving from the established vocabularies: axioms are Clean Architecture's entities, flows its use-cases, harnesses the hexagonal adapter at its anti-corruption seam, services the imperative shell's inner tier - close translations, and traps where they differ: a DDD entity carries identity and mutable state, which the axioms layer forbids, and a naive GoF adapter leaves the foreign model in charge, which is the exact violation the harness's translation obligation names.
...
A translation legend maps the two diagrams' labels onto this document's vocabulary. The Layers diagram's **Application Runtime** band is the composition root; its **Features** row is the Flows layer, its **Domain Rules** row the Axioms layer, and its **Shared** column the pure core (zero outside dependencies; arrows point from domains into it, never back). The Shared column is one example of a core domain, and the diagram deliberately leaves its internal file structure undrawn: a core slices vertically or horizontally like any domain (the Slicing direction section). The General diagram draws each layer with a producer facet and a consumer facet, example artifacts on each (`web-api`, `gateways`, `streams`, `rules` down the producer side; `UI`, `presenters`, `processes`, `models` down the consumer side); the API bands face the peer domain, the Events bands sit on the outer edges, and the bottom half zooms into the Flows band to draw the Pipe-Event-Stream cycle with its Active/Passive matrix. Along the U, data may be enriched, re-shaped, or filtered by data drawn from Axioms. There are no upward arrows anywhere; within a subcolumn every dependency arrow points down. The runtime band's composing arrow recurses: within the Feature column the domain itself plays the Application Runtime role for its subdomain columns, the composing arrows fanning out per subdomain, and each subdomain column realizes its four layer rows as one artifact box each, one file per layer.

### Subdomains and nesting

A layer is a classification of code. The file name carries the layer and the unit, the reference graph draws the domain and its subdomains, and a directory is where a developer chose to keep a file. The graph is the concern ontology, so the tree is free: mirror it to the concerns for legibility or leave it flat, and the reading is the same.

Nesting is an ownership placement, and three consequences follow:

- **Siblings are peers.** Between two subdomains of one parent, the full cross-domain discipline applies: consumable surfaces, the flow crossing, the graded lateral mounting.
...
- **Information flows downward**: Services → Harnesses → Flows → Axioms
- **Awareness flows upward**: Axioms define interfaces for Services to implement; Axioms do not import Services
- **Activity progresses top-to-bottom** through all layers for any given request
- Inner layers (Axioms, Flows) **must not import** outer layers (Harnesses, Services)
- Dashed dependency lines = weak/optional coupling; red dashed = inadvisable, avoid. The diagram draws the red links at both outer rows: service-to-service and harness-to-harness across units. Its cross-unit links at Flows are ordinary solid dependencies (the designed crossing), and at Axioms weak ones (vocabulary reference)

### Crossing layers

The downward edges (Services → Harnesses → Flows → Axioms) are both control flow and source dependency: a request enters at a Service, calls down through the layers, and imports point the same way. The upward edges are control flow only, an inner layer invoking something an outer layer provides, and on those edges the source dependency is inverted.

Inversion here is purely structural - no DI container, no service locator. It means: **an inner layer declares what it needs as parameters, and the layer above supplies them.** A flow that needs the current route takes it as an argument; the harness that wires the router hands it down. A flow that needs to navigate takes a `navigate` callback; the harness supplies the concrete one. The "port" is usually just the parameter list, a structural contract, and earns a named interface only when the contract is large enough to be worth naming. The supplying happens by ordinary composition - the outer layer calls the inner one within its own scope and passes its values down - so the composition is the wiring.
...
Corollary, the **relocation rule**: if an inner layer finds itself needing an *outer layer's module* (a flow importing a service, an axiom importing a harness), that is misplaced logic. Split it. The pure part, a function of plain values, moves inward to where its dependencies actually are; the binding part, which reads the outer module and feeds the pure part, moves outward to the Harnesses layer, whose defined job is exactly this binding. After the split nothing imports upward, because the inner part now depends only on values and on ambient mechanism. **Ambient mechanism** is the substrate the host already supplies everywhere - the language runtime, a channel-conforming reactive primitive, the rendering runtime: code uses it in place, it never forms an import edge to an outer layer, and it exempts nothing from a layer's own purity rules.

The relocation rule's standing form is the **harness posture**: coupling gear lives in the harness by default. Try/catch, promise plumbing, host lookups, caches over reads, and format decoding appearing inside a flow or a service is the harness knocking; leaving such gear inline is the active decision, and it carries the same one-line justification an inadvisable mounting does. The posture governs defaults, and every legality stands: a service still calls a flow directly wherever no gear sits between them, a unit with no gear needs no harness file, and the question a reader asks flips from "do I need a harness yet" to "which harness does this gear belong to". A harness earns its file by converting, absorbing, or caching something; a pass-through that translates nothing is the naive-wrapper ceremony the indirection rule already refuses. Where the world initiates - a host that mounts and drives the domain's behavior - the gear concentrates into one mounting engine per protocol, treated under Libraries.

Downward, the ordering protects the activity path: a request descends through every layer's work. A bare downward *reference* at any distance (a Service naming an Axiom type) is deliberately unregulated - a type is stateless vocabulary, and regulating reference distance would demand hops that home no decision, which the indirection rule refuses.

### The two axes and the projection curve

The Layers diagram draws two axes over the whole map. The **Activity axis** (vertical) is the probability of encountering an active side: Services predominantly drive (they make the outgoing call, mount the UI, boot the SDK), Harnesses actively drive the external APIs they wrap, Flows are the pivot where active production meets active consumption, and Axioms are fully passive - pure invariants invoked on demand. The **Generalization axis** (horizontal) is how few specifics the code holds: more general code carries fewer constants and less concrete logic. The diagram's diagonal curve is a mental frame that limits divergence between the two: each layer's code should project onto the curve, so highly active code is specific and highly general code is passive. Code far off the curve (a very general module that actively drives, a very specific one that only waits) is a placement smell.
...
The full loop between two domains (the Pipe-Event-Stream association): a consumer flow `call`s a Generator, which `trigger`s the producer flow, which `update`s a Stream, which `notify`s the consumer flow. A domain is the active producer at the Stream it updates, the passive producer at the Generator it is triggered through, the active consumer at the Generator it calls, and the passive consumer at the Stream that notifies it.
...
A **unit is the files sharing one name, dedicated to one concern-part**.
Examples:
- `back-nav.harnesses.tsx` and `back-nav.harnesses.css`;
- `a-star.flows.cpp` and `a-star.flows.h`.
These are one unit composing itself, and intra-unit imports carry no restriction beyond the layer order, which binds wherever the files sit. A different name is a different unit, and the cross-unit rules (the lateral service-to-service and harness-to-harness smells among them) act on the names: to draw a boundary between two things sharing a name, give each its own. A unit is a concern as much as a subdomain is; the name carries the unit, the graph draws the subdomain, and a directory is how the files are grouped. Group files however a crowded domain reads best, a grouping that stays free because the classification reads the name and the graph. A concern becomes a subdomain when it grows a surface, the thing a reference crosses to enter it. The spec never mandates decomposition; structure starts coarse and splits when a boundary becomes real. An effect-only import (a bare import for its side effect) reaching past its unit hides an effect in the graph and is a smell; effect composition by import belongs to the composition root.

### Slicing direction

A domain's files partition one way or the other. **Vertical** slicing is concern-first: units named for their concern, each file carrying its layer as a name suffix, the layers meeting inside each concern's column. **Horizontal** slicing is layer-first: directories named for their layer, each holding plain concern-named files, the concerns meeting inside each layer's row. Both are spellings of the same classification: the layer rides the suffix in one and the layer-named directory in the other. The unit rides the concern name in the vertical spelling; in the horizontal spelling a plain file directly in a layer row joins that row's shared aggregate - the row's counterpart of the bare reserved-name file, with the row's `index` as the aggregate spelling - and a directory inside a row labels a unit the way a unit directory does. Layer rows are rows of one subdomain, never subdomains. Every rule reads the classification, which both spellings carry in full.

The choice is judgment the architecture leaves to the domain, one domain at a time. Highly specialized concerns slice vertically: each concern owns its column, and a boundary between two concerns is a rename away. Logic entangled by design - units leaning on one another across layers, as a library's internals routinely do - slices horizontally: the layer rows carry the structure, and the imports cross inside a row. A concern partition over the same code would put a wall through every lean.

A wrong slice shows up in the graph: imports that climb the layer order accumulate between siblings, and the cluster is the pressure of a partition fighting its dataflow. The remedy is to re-slice - merge the fighting siblings into one domain whose layer rows carry the structure, or move each sibling to the layer its imports say it holds. The machine gathers the pressure and surfaces the cluster; choosing the new slice is the reviewer's decision, and the enforcement tiers place it there.

### What a file is fixes its classification

A stylesheet is code: it composes through ports, scopes on boundaries, and layers internally, so it classifies by the layer it occupies, obeys that layer's rules, and belongs to the unit it is co-located with. A runtime-context marker (`.server`, `.client`) and a build-convention compound (vanilla-extract's `.css.ts`) are colorings the classification sees through; the layer rides the suffix before them. A pure-data asset (an image, a font, a media file) carries no behavior and resolves to a value on import, so it is vocabulary, classified as an Axiom: importable from any layer, never a service, surface-gated across domains.
...
A foreign owner's contract (a shared library's tokens, an external system's types) is referenced by name at the layer its kind permits: a type at any layer, a presentation token at Harnesses and above and never in an Axiom, an implementation detail (a utility-class string) nowhere.

The domain's own contract keeps its owner when its format is a foreign host's: an options schema, a manifest entry, a wire shape is the domain's declaration written in the host's grammar, owned at Axioms like any vocabulary. An identifier that is at once the host's construct and the domain's terminology - a rule ID, a route name, a job name - splits along that line: the name is declared once at Axioms, and the host-shaped object it keys is the mounting engine's product. Spelling such a name in more than one place is the tell that its owner is missing.

### Cross-cutting concerns are blocks

A concern that owns state, encodes a policy, or authors markup more than one consumer would otherwise re-author cannot escape its owning domain as a free-floating hook; each consumer mounting it would re-instance ownership. The owning domain absorbs it into a **composition block** - a service - and the root composes it once, with consumer content composed in; other domains contribute data and behavior contracts. A helper that owns nothing and returns a pure value over ambient mechanism or already-published state stays directly consumable as a flow; the discriminator is re-ownership: does consuming it as a bare hook force each consumer to re-instance or re-decide what one owner should hold?
...
**All data starts and ends at the root.** A root-side producer feeds a producer Service; data descends the producer domain's layers to its flow, crosses to the consumer domain's flow over the channel, ascends to the consumer Service, and exits at the root-side consumer - a U-shaped detour, enriched along the way from the Axioms layer. The same root is both faces of its U.
...
Domains communicate intra-root, by reference, through the flow channel; a reference is valid only inside one runtime's memory, so a U never spans two roots. Roots communicate inter-root by serialization through the **transport plane** at the Services layer: a server-to-client flow is two Us chained through transport, each root the alpha and omega of its own plane. This is a deliberate rejection of location transparency: a network hop is never disguised as a local reference.

---

## The cross-domain data path

The flow ↔ flow channel is the only cross-domain crossing, so every compliant path has one spine: source layer descends to its own flow (import-and-call), crosses the channel, ascends the consumer's layers (by inversion) to the target. Axioms are data sources, drawn from and never written to; Services are where data exits; Flows are the pivot. An effect never crosses at all: only data does - a request on the API surface, a value on the Events surface - so an effect stays contained to the domain that owns it.

What needs no path: a pure, total, effect-free thing (a name, a shape, a pure total function) is referenced directly by name through the surface - there is no state to go stale and no effect to contain. Whether such a thing is an axiom or a flow is the separate invariant-versus-process axis; purity earns the direct reference, the axis sets the layer.

Effect regulation follows the same ownership, because an effect is a write to some target. A write to state the domain owns is legal at the layer that owns it - a reactive primitive's setter called inside the flow that created it is the domain exercising its own state - and needs no containment of its own, because what the architecture contains is the write's *observability*: nothing outside the owner sees the mutation except through published, immutable, change-gated channels. A write to the outside world binds at the outer layers, where the world's shape is bound. A write to another domain's anything is re-ownership and never happens; the consumer invokes the owner's published operation instead. Axioms own no state, which makes them the one effect-free floor. The incidental micro-effects a host runtime makes unavoidable (allocation, module evaluation, memo caches) sit below the architecture's resolution and go unregulated.

---

## Outcome model

Two parties speak different outcome vocabularies. The platform knows only **runs or throws** - it observes termination shape alone. The domain knows only its **typed business outcomes** - branches with meaning. Architectures that let the machine's binary leak into the domain's branching route a valid pessimistic outcome (a not-found, a validation failure) as a soft value or a hard exception by implementation accident. ELDA makes the layer stack a **bidirectional translator** between the two vocabularies:

- **Inbound**: a throwing external API is wrapped at Harnesses into a typed value - one of the domain's meaningful branches. From there outcomes travel the normal data paths; there is no separate error channel, no exception in the call graph, and no branch designated as semantically wrong. Async and throwing are the same shape mismatch, translated at the same boundary.
...
Ownership of outcomes: the Harness touching a throwing API owns the inbound conversion; each domain owns its outcome shapes as vocabulary declared at Axioms and exposed through the surface; a consumer handles the producer's typed branches and never reaches in to catch a raw throw. Exhaustive union checking is an ergonomic lift that makes the bookkeeping cheap; the substance is the translation itself, which runs the same on tagged values and convention.

---

## Concurrency: neutralize the colorings

Async/await splits a codebase into two incompatible function kinds whose agreement propagates transitively through the call graph. ELDA neutralizes the coloring into one kind: every asynchronous operation is wrapped at the Harnesses layer into a single-emission channel, and inner layers receive only channel protocol, never a raw promise. The channel's storage shape (whether a late subscriber sees the settled value) is the usual orthogonal choice. Execution context (compile-time, server-before-hydration, client-after) is the same kind of coloring, neutralized the same way: each context gets its own composition root under identical rules, and none is granted a structural exception. "Static" means stateless and behaviorless, never compile-time.

The temporal composition of a multi-step process, worked once on an outbound request (`fetch` in a web runtime is the canonical instance). The harness owns the one conversion: the call, its exception absorption, and its status mapping become a typed outcome delivered on a channel. The flow hinges on that outcome as a plain value, in either sanctioned shape. Push-shaped, the flow's step ends at "request issued, state pending", and the outcome's arrival re-enters the next step through the Pipe-Event-Stream loop, synchronously. Pull-shaped, the flow is a language generator yielding a request description and receiving the outcome at the yield, so a many-step process keeps its lexical sequence while a service drives it and holds the awaiting. The waiting always lives between flow activations or in the driver; no flow contains the platform's spelling of it.

---

## State model

State is local to the domain and layer where it originates; there is no global state construct and no shared mutable reference between domains. A reactive primitive holding local state inside a flow is that flow's own affair. State that must cross a boundary is **published**: the producer emits values through a channel, consumers hold a subscription reference - a reference to the channel, never into producer state - and published values are immutable (see Channels). This forecloses the three failure classes of shared state: write contention (each location has one owner; published values cannot be written), implicit reflow (propagation is explicit channel delivery), and stale snapshots (a state-shaped channel delivers the current value at subscription and every update after).

Scope honesty: a co-change invariant spanning several axioms has one owner - a domain whose flow performs the whole change as one published operation - and transactional atomicity itself is a terminal-tier capability (the storage or backend transaction), reached through transport or storage domains. ELDA locates the invariant; it does not manufacture the guarantee.
...
A third-party library is a black box: ELDA does not see or govern its internals, only its surface as consumed. Consumption follows the same rules as any foreign system. An API that already conforms to the concepts (reactive accessors, value-returning hooks, pure callbacks, idempotent on import) is consumed directly and orchestrated by flows; externality alone triggers nothing. A mismatched shape is adapted at the boundary layers. At a foreign boundary the Harness's obligation is **translation**: the domain declares its own axiom vocabulary and the Harness maps the foreign shape into it - a wrapper that merely re-shapes, leaving the foreign model in charge of the domain's types, has adopted the foreign model behind a compliant import graph. Between in-repo peer domains no such obligation exists: referencing a peer's vocabulary through its surface is the designed path, because the owner is inside the system and renames propagate by type. A library's own vocabulary is a foreign owner's contract, referenced by name and never re-declared. Wiring substrates are bounded by the bar stated under Positioning.

One recurring boundary case: a validation library whose schema factory co-locates the data shape with a throwing validator produces an effectful object, and invoking that factory at the Axiom layer lands the object on the pure floor. The Axiom holds the shape; the factory invocation and its throwing surface belong outward, at the Harness that translates the boundary. The library will not separate the two structurally, so that wall is the architecture's to build.

The reverse boundary is the **host protocol**: a framework that mounts and drives the domain's behavior instead of being called by it - a lint host mounting rules, a UI framework mounting components, a message broker driving handlers, a game loop driving systems. The protocol's grammar (registration shapes, lifecycle callbacks, the host's own object model) is coupling gear, and smeared through every mounted piece it buries the domain logic in host ceremony; the tell is that a reader cannot say which lines are the piece and which are its feeding. The gear extracts once, into a **mounting engine** - a harness. The flows export host-free definitions: declarative data plus judgment functions over plain values, written as if no host existed. The engine consumes those definitions downward, registers them in the host's grammar, translates the host's callbacks into events phrased in the domain's own vocabulary, and supplies each definition its ports. A host object a definition must hand back (a report target, a render slot, a span) travels through it as an **opaque token**: passed in with an event, returned to a supplied port, never opened. The direction is the point - the engine imports the definitions and the definitions import nothing above their layer, so the mounted behavior needs no port to the gear that drives it, and the host can be replaced, or faked at a test bench, by re-mounting the same definitions. One engine per protocol is the indirection rule satisfied; a wrapper per mounted piece is the same rule refused, and it is the shape the smear grows back into.

Retrofitting an existing host boundary onto this shape follows the order a live gate can hold: build the engine beside the mounted pieces, migrate the simplest piece first and the most host-entangled piece last, keep the machine checks green at every step, and finish by moving the protocol's names and contract shapes to their owner at Axioms.

---

## The type-level language

A type-level language (compile-time-gated type computation) is a second computational language, and ELDA is self-similar into it. The static principles recurse in full: a plain shape is a type-level axiom, a type-level function (conditional, mapped, recursive) is a type-level flow, exported types are the surface and unexported helpers are private, ambient declarations and merges are owned side-effects, and a computation that resolves to an error-shaped type instead of the compiler's bottom is the inbound outcome translation, recursed. The dynamic principles relocate to the meta-runtime whose clock ticks during reference resolution (largely at authoring time): navigation ergonomics become resolution paths, execution frequency becomes instantiation cost, state-over-time becomes state-over-resolution. In practice type-level code shadows the value-level layer it types; standalone type-level domains live mostly in libraries, which are opaque anyway.
...
| Harnesses | Harness, Decorator, Proxy, Component |
| Flows | Command, Mediator (coordination policy with one owner), Chain of Responsibility |
| Axioms | Strategy, Composite, Specification, Value Object |
...
- **LAYER.1** Inner layers never import outer layers: the layer order is Axioms → Flows → Harnesses → Services, and an import against it is a violation regardless of path form.
...
- **LAYER.3** An inner layer needing an outer layer's module is misplaced logic: split it - the pure part (a function of plain values) moves inward, the binding part moves out to Harnesses.
- **LAYER.4** Flows and Axioms traffic only in values: no async/await or promise chains, no try/catch, no throwing calls. Async and throwing shapes are wrapped at Harnesses into channel-conforming values; the imperative shell is the only other legitimate host for them (ROOT.2). A `try/finally` with no catch clause is completion protocol, and a flow holds one where a language generator restores the flow's own state on early return; the license covers cleanup of owned state and never error handling.
- **LAYER.5** The harness is where coupling gear lives. Shape mismatch with the concepts creates the gear (imperative interfaces, throwing APIs, async or promise control flow, mutable global state, request/response), wherever the mismatched shape arrives from: a wire, a platform SDK, a host API, an in-process foreign model such as an AST. Externality alone never creates it - an API that already conforms is consumed directly and orchestrated by flows - and gear found inline in a flow or a service moves to the harness or carries a per-instance justification; a harness holding no conversion is ceremony (META.5).
- **LAYER.6** At a foreign boundary (a wire format, a platform SDK, a third-party API) the Harness translates the foreign shape into the domain's own axiom vocabulary; re-shaping that leaves the foreign model in charge of the domain's types is conformity, a violation. In-repo peer domains are exempt: referencing a peer's vocabulary through its surface is the designed path.
- **LAYER.7** A layer is a classification carried by the file name, and structure is read from names and the reference graph. Directory organization is free, and the classification is independent of where a file sits: the property holds by construction.

### CHANNEL - between domains

- **CHANNEL.1** Domains communicate exclusively through Streams and Generators attached at the flow layer; the flow ↔ flow channel is the only cross-domain crossing, and effects never cross - only data does.
...
- **SURFACE.2** The consumable surface carries flows and vocabulary and never exposes services or harnesses; services are published only on the runtime-composition surface, reached by composition roots - and, under OWNER.5's graded exception, by a peer service mounting the block at that surface, never past it.
...
- **SURFACE.5** A unit is the files sharing one name, dedicated to one concern-part: intra-unit imports are unrestricted except by the layer order (LAYER.1); cross-unit rules act on the shared names, and decomposition is never mandated. A cross-name value reference - into a sibling unit, a nested domain, a peer - lands at the consumer's own layer; a landing below it is a diagonal no row of the diagrams draws, direct and republication-laundered alike. A layer file that republishes a name by named re-export adopts it at its own layer and owns it toward its consumers; a surface republishes without adopting, so a reference through a surface is judged at its landing. An effect-only import reaching past its unit is a smell; effect composition by import belongs to the root.
- **SURFACE.6** What a file is fixes its classification: a stylesheet is code and classifies by its layer and unit; a pure-data asset is vocabulary, classified as an Axiom - importable from any layer, never a service, surface-gated across domains.
...
- **OWNER.5** Lateral composition between outer-layer units of peers is graded (a domain composing its own nested domains is self-composition, ROOT.7): a named slot port the root fills is the designed form; a service unit mounting a sibling block - another service unit of its domain, or a peer domain's block at its runtime-composition surface, never past it - is inadvisable and carries a per-instance justification; re-authoring a sibling concern from its flows re-owns it and is the violation (OWNER.4). Between Harnesses of different units the same grading holds, without the surface case since harnesses are published on no surface: the layer above composes the two bindings, or they co-locate into one unit.
- **OWNER.6** A foreign owner's contract is referenced by name at the layer its kind permits: a type at any layer; a presentation token at Harnesses and above, never in an Axiom; a foreign implementation detail nowhere.
...
- **ROOT.5** Roots communicate inter-root only through the transport plane at the Services layer, by serialization; the flow channel never spans two roots.
...
- **Generator**: a plain function or accessor on the surface. A language `function*` only where consumers genuinely pull a sequence step by step; a Subject-backed intent channel only where real producer-side aggregation exists (multiple consumers, debouncing, conditional flows). "On event, call the producer's published flow" needs no reactive plumbing.
- **Async operation**: wrapped at Harnesses into a single-emission channel, state-shaped if late subscribers must see the settled value. Never a raw Promise past the Harness.
