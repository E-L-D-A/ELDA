...
- **SURFACE.5** A unit is the files sharing one name, dedicated to one concern-part: intra-unit imports are unrestricted except by layer rank, which LAYER.1 binds regardless of path form; cross-unit rules act on the shared names (a legacy unit directory counts as one name), and decomposition is never mandated. An effect-only import reaching past its unit is a smell; effect composition by import belongs to the root.
