...
export const asyncFn = () => 'ELDA LAYER.4: an async function colors this layer. Express the wait as a channel step: a harness converts the call into a typed outcome, a subscription or a yield delivers it here as a plain value, and the awaiting lives in the service that drives.';
export const awaitExpr = () => "ELDA LAYER.4: await is the platform's spelling of waiting, and it stays out of this layer. Express the wait as a channel step: a harness converts the call into a typed outcome, a subscription or a yield delivers it here as a plain value, and the awaiting lives in the service that drives.";
export const forAwait = () => 'ELDA LAYER.4: for-await consumes an async iterable in place. Let a harness convert the iterable into a subscription delivering plain values, and consume those here.';
export const tryCatch = () => "ELDA LAYER.4 (Outcome model): a catch is the platform's error branch, and outcomes travel as typed branch values here. Let a harness absorb the throw into an outcome and branch on the value; a finally with no catch clause is completion protocol and stays legal in flows.";
...
export const axiomState = (name) =>
  `ELDA LAYER.4: an axioms file holds pure domain invariants and owns no state, and the module-level mutable binding '${name}' is state in the one layer that holds only invariants. Move the state to an outer layer that owns it, and keep the invariants and pure functions here.`;
...
export const surfaceCarriesUseCases = (targetLayer, domainAlias, roleChain) =>
  `ELDA SURFACE.2: the consumable surface carries flows + vocabulary; '${targetLayer}' belongs to the runtime-composition surface (${domainAlias}/${roleChain}/services), reached only by its composer.`;
