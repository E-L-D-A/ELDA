...
import * as msg from '../axioms/messages.js';
import { LAYER_RANK, isComposer, isServicesSurface, rel, unitOf } from '../axioms/model.js';
...
//   SURFACE.2  a consumable surface carries flows and vocabulary, never services or harnesses;
...
export function judgeImport(role, t, ownershipAlias) {
  ...
  if (role.kind === 'surface') {
    ...
    // A consumable surface carries flows and vocabulary only; services and harnesses belong to the runtime-composition surface, which the `services` file realizes and may reference freely.
    ...
    if (role.surface !== 'services' && (t.layer === 'services' || t.layer === 'harnesses')) {
...
// That blinds LAYER.1 outright, and the blinding is one-directional in the dangerous direction - the consumable surface legally carries flows (SURFACE.2), so an axioms or flows file can take an outer-layer binding straight through its own barrel and no per-file rule sees the inversion.
...
export const LATERAL = {
  services: { remedy: 'supply it as a named port from the composition root, or lift the shared logic into a flow.', crossSurface: true },
  harnesses: { remedy: 'let the layer above compose the two bindings, or co-locate them into one unit.', crossSurface: false },
};
...
// So a binding the root takes that lands off the services row marks a service smashed into the root: the twin, one rank out, of a service reaching a foreign flow.
...
// A flow does not become a service by transiting a services file, so publication is judged at the seam where the decision was made, and not once per consuming root.
