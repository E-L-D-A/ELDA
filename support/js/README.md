...
The page states its own condition: the first load waits behind a spinner, a scan that found nothing explains itself with the derivation's notices, a render fault names itself with its stack instead of leaving a distorted board, and a failed rescan banners its diagnosis while the last good graph stays up.
...
What the derivation decides around, it also reports. An unreadable config, an ambiguity it refuses to guess at, a tree with no forest to find - each becomes a notice, printed once at derivation and carried on the scan's options, so the viewer surfaces the same diagnoses the console got. A misconfigured tree is answers all the way down - a null forest, an empty file list, a notice naming why - never a crash. And where the scan itself fails inside the lint host, the degradation is declared rather than silent: every file of the faulted app reports the fault through `elda/imports`, the remaining rules fall back to file-name classification, and the gate reads visibly narrower instead of quietly clean (META.6).
