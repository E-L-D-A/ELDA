...
// The scan, the walker, and the app-root resolution sit at ranks above this one (core's services, core's harnesses), so the rules declare the needs here and enforce's services file supplies the implementations by ordinary composition (LAYER.2); this module touches no filesystem and reaches nothing upward.
...
// The taking side is the sharper of the two, because the consumable surface legally carries flows (SURFACE.2): a file at axioms or flows rank that imports its own barrel can take an outer-layer binding through it, and LAYER.1 - a per-file rule reading the specifier - sees a rankless surface and passes. The landing walk does not cover the gap, since it grades flows landing below the consumer's rank and this inversion lands above.
...
const noHarnessCoupling = lateralCoupling('harnesses');
...
//   acrossDomains      a foreign domain's surface - a laundered cross-domain crossing off the flow row (default 'error').
...
const noAsyncInner = {
  create(context) {
    const opts = options(context);
    const { role } = graphClassify(filenameOf(context), opts);
    if (role.kind !== 'domain' || (role.layer !== 'axioms' && role.layer !== 'flows')) return {};
    ...
    return {
      ...
      TryStatement: (node) => {
        if (node.handler == null && role.layer === 'flows') return;
        context.report({ node, message: msg.tryCatch() });
      },
...
// A module-level `let`/`var` in an axioms file is state at the pure rank, whether exported or private behind an accessor; the decidable slice is the mutable binding itself, and whether a const collection is mutated stays with review, the same split no-mutable-surface declares for CHANNEL.4.
const noAxiomState = {
  meta: {
    schema: [{
      type: 'object',
  ...
  create(context) {
    const opts = options(context);
    const { role } = graphClassify(filenameOf(context), opts);
    if (role.layer !== 'axioms' || role.asset) return {};
    return {
      Program: (program) => {
        for (const n of program.body ?? []) {
          ...
          for (const d of decl.declarations ?? []) {
            context.report({ node: decl, message: msg.axiomState(d.id?.name ?? 'it') });
          }
        }
      },
    };
...
export const rules = {
  ...
  'no-harness-coupling': noHarnessCoupling,
