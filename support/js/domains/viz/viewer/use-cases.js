// ---------------------------------------------------------------------------
// The viewer subdomain's shared use-cases base: the session state, the board's derived reading and its composer-filled ports, the placement derivations, and the drawn-edge derivation.
// Every unit of the subdomain reads this base vertically, which is what a bare layer file is for; state lives behind accessors so no live mutable binding crosses the module boundary (CHANNEL.4), and the visibility sets are constants mutated through their own API.

import { ROWS } from "./entities.js";
