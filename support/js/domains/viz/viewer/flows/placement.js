...
import { ROWS } from "../axioms/index.js";
    ...
    if (collapsed.has(domain))
      return {
        ...
        band:
          ...
          (r.layer === "services" || r.layer === "axioms"),
      };
    return {
      area: "domain",
      core,
...
// A domain-wide band: the bare composer (the sub-root cap) or the bare axioms file (the shared base), lifted out of the cake and spanning its (sub)domain. Edges anchor these vertically only.
...
export function isBarFile(f) {
  ...
  return (
    ...
    (p.row === "services" || p.row === "axioms")
  );
}

// A services composer (a sub-root band) threads away like a surface: an edge into it re-emerges as edges to the modules it re-exports, so arrows land on the real carriers behind the composition layer. `data().landings` already carries the composer's onward hop.
